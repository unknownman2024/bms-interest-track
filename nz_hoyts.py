...
SESSIONS_URL_TEMPLATE = "https://apim.hoyts.co.nz/nz/cinemaapi/api/sessions/{cinema_id}"
...
async def main():
    ...
    for show_date, data_dict in sessions_by_date.items():
        diff_path = f"Newzealand Data/{show_date}-data.json"
        ...
        summary_path = f"Newzealand Data/{show_date}-summary.json"
        with open(summary_path, "w") as f:
            json.dump(summary_list, f, indent=2)
        print(f"📊 Summary saved to {summary_path}")

        # ---- UPDATE MOVIE DATES ----
        movie_dates_path = "Newzealand Data/movie_dates.json"
